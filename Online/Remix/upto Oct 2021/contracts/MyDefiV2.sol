...
contract MyDefiV2{
    ...
    function addLiquidityByEther_Dia(
        ...
        IUniswap(_uniswap).addLiquidityETH{
            value: msg.value // max amount of ether we want to add as liquidity
        }(
            _diaToken,          // pool dia token address 
            diaAmount_,         // max amount of dia tokens we want to add as liquidity
            minDiaAmount_,      // min allowed dia token price to reach before tx reverts, <= diaAmount_
            minEtherAmount_,    // min allowed Weth price to reach before tx reverts, <= msg.value 
            lptReceiver_,       // recipient of the liquidity provider tokens
            deadline_           // unix timestamp after which the tx will revert
        );
    }
