...
contract RoboDodgeNFT is ERC721 {
    ...
    constructor() ERC721("RoboDoge NFT", "RDN") {
        _owner = msg.sender;
    }
    ...
    function disableTokenTransfer(uint256 tokenid_)
        external
        tokenOwnerOrApproved(tokenid_)
    {
        tokenData[tokenid_].transferEnabled = false;
    }

    function enableTokenTransfer(uint256 tokenid_)
        external
        tokenOwnerOrApproved(tokenid_)
    {
